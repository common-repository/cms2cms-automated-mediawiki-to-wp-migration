...
/**
 * Class CmsPluginFunctionsMediaWiki
 */
class CmsPluginFunctionsMediaWiki
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback" : "//wordpress.org/support/plugin/cms2cms-automated-mediawiki-to-wp-migration/reviews",
    ...
    {
        $cms2cms_mediawiki_access_login = get_option('cms2cms-mediawiki-login');
        $cms2cms_mediawiki_access_key   = get_option('cms2cms-mediawiki-key');

        return array(
            'email'     => $cms2cms_mediawiki_access_login,
            'accessKey' => $cms2cms_mediawiki_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_mediawiki_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-mediawiki-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-mediawiki-key', $key);

            delete_option('cms2cms-mediawiki-login');
            add_option('cms2cms-mediawiki-login', $login);

            delete_option('cms2cms-mediawiki-depth');
            add_option('cms2cms-mediawiki-depth', $bridge_depth);
    ...
    {
        return $this->__('MediaWiki', 'cms2cms-mediawiki-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-mediawiki-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-mediawiki-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-mediawiki-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_mediawiki_logout')
                && $_POST['cms2cms_mediawiki_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
